    ...
    public function page() {
      ...
      switch($action) {
        case 'places':
          ...
          $trash_num = $wpdb->get_var($wpdb->prepare('SELECT COUNT(*) FROM %s WHERE trash = TRUE', $pTable));
          $active_num = $wpdb->get_var($wpdb->prepare('SELECT COUNT(*) FROM %s WHERE trash = FALSE', $pTable));
    ...
    <div class="tablenav-pages">
      <?php $page_links_text = sprintf( '<span class="displaying-num">' . __( 'Displaying %s&#8211;%s of %s', SAM_DOMAIN ) . '</span>%s',
        ...
        case 'items':
          ...
          $trash_num = $wpdb->get_var($wpdb->prepare('SELECT COUNT(*) FROM %s WHERE (trash = TRUE) AND (pid = %s)', $aTable, $item));
          $active_num = $wpdb->get_var($wpdb->prepare('SELECT COUNT(*) FROM %s WHERE (trash = FALSE) AND (pid = %s)', $aTable, $item));
          if(is_null($active_num)) $active_num = 0;
          if(is_null($trash_num)) $trash_num = 0;
          $all_num = $trash_num + $active_num;
